...
def test_tt_prediction(
    ...
    val_genn = GenModel()
    val_model_configs = val_genn.generate_model_configs(
        num_model_data=val_data_points)
    ...
    for m_config in tqdm(val_model_configs):
        ...
        train_batch_numbers = math.ceil(
            data_size_val / batch_size_val) * epochs_val
    ...
    if plot:
        ...
        plt.scatter(x, cal_score(y_val_preds_batch,
                    real_time_process_first_batchs))
        ...
        plt.scatter(
            ...
            cal_score(
                y_val_preds_setup,
                np.array(real_time_batchs) -
                np.array(real_time_process_first_batchs),
            ),
        )
        plt.plot(x, [0.15] * len(x), c="r", linewidth=10)
        plt.plot(x, [-0.15] * len(x), c="r", linewidth=10)
...
def demo():
    ...
    test_tt_prediction(batch_model, setup_model, scaler,
                       plot=True, val_data_points=1)
